...
class TestServiceEventSchedulingModel(TestCase):
    ...
    def test_all(self):
        ...
        self.sch1.due_date = start + timezone.timedelta(hours=12)
        self.sch1.save()
        self.sch2.due_date = timezone.now() + timezone.timedelta(hours=2 * 24)
        self.sch2.save()
    ...
    def test_upcoming_and_overdue(self):
        start, end = today_start_end()
        self.sch1.due_date = start + timezone.timedelta(hours=36)
        self.sch1.save()
        self.sch2.due_date = timezone.now() - timezone.timedelta(hours=2 * 24)
        self.sch2.save()
...
class TestServiceEventSchedulingEmails(TestCase):
    ...
    def test_send_notice(self):
        now = timezone.now()
        self.sch2.due_date = timezone.now()
        self.sch2.save()
    ...
    def test_schedule_notice(self):
        ...
        tasks.schedule_service_event_scheduling_notice(self.notice, next_run)

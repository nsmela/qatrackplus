...
class BaseReport(metaclass=ReportMeta):
    ...
    def to_pdf(self):
        ...
        try:
            ...
            # WeasyPrint failed for some other reason, fall back to Chrome.  Log
            # the traceback: the fallback is silent, so without it a broken
            # report just looks like a rendering bug.

...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        create_permissions(get_app('qa'), get_models(), 2 if settings.DEBUG else 0)

        can_review_non_visible_tli = Permission.objects.get(codename='can_review_non_visible_tli')
        can_review = Permission.objects.get(codename='can_review')

        for group in Group.objects.filter(permissions=can_review):
            group.permissions.add(can_review_non_visible_tli)
            group.save()

        for user in User.objects.filter(user_permissions=can_review):
            user.user_permissions.add(can_review_non_visible_tli)
            user.save()
    ...
    def backwards(self, orm):
        ...
        for group in Group.objects.filter(permissions=can_review_non_visible_tli):
            group.permissions.remove(can_review_non_visible_tli)
            group.save()

        for user in User.objects.filter(user_permissions=can_review_non_visible_tli):
            user.user_permissions.remove(can_review_non_visible_tli)
            user.save()

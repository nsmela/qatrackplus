...
from qatrack.qa.models import MULTIPLE_CHOICE
...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        for test in mc_tests:
            ...
            for ti in list(TestInstance.objects.filter(unit_test_info__test=test).exclude(value=None)):
                try:
                    string_val = test.choices.split(',')[int(ti.value)]
                    TestInstance.objects.filter(pk=ti.pk).update(string_value=string_val, value=None)
                except IndexError:
                    raise ValueError("Migration 19 failed on TestInstance %d. %d is no longer a valid choice" %(ti.pk, ti.value))
        ...
        for test in mc_tests:
            for ti in list(TestInstance.objects.filter(unit_test_info__test=test).exclude(string_value=None).exclude(string_value='')):
                try:
                    ...
                    TestInstance.objects.filter(pk=ti.pk).update(string_value=None, value=val)
                except IndexError:
                    raise ValueError("Backwards migration 19 failed on TestInstance %d. %s is no longer a valid choice" %(ti.pk, ti.string_value))
